...
def final_net(singer_label, f0_notation, phones):
    ...
    deconv1 = tf.nn.relu(deconv2d(conv12, [config.batch_size, 2, 1, config.wavenet_filters], name = "F_dec1") +  conv7)

    deconv2 = tf.nn.relu(deconv2d(deconv1, [config.batch_size, 4, 1, config.wavenet_filters], name = "F_dec2") + conv6)

    deconv3 = tf.nn.relu(deconv2d(deconv2, [config.batch_size, 8, 1, config.wavenet_filters], name = "F_dec3") + conv5)

    deconv4 = tf.nn.relu(deconv2d(deconv3, [config.batch_size, 16, 1, config.wavenet_filters], name = "F_dec4") + conv1)

    deconv5 = tf.nn.relu(deconv2d(deconv4, [config.batch_size, 32, 1, config.wavenet_filters], name = "F_dec5") +  inputs)
...
def phone_network(inputs):
    ...
    deconv1 = tf.nn.relu(deconv2d(conv12, [config.batch_size, 2, 1, config.wavenet_filters], name = "P_dec1") +  conv7)

    deconv2 = tf.nn.relu(deconv2d(deconv1, [config.batch_size, 4, 1, config.wavenet_filters], name = "P_dec2") + conv6)

    deconv3 = tf.nn.relu(deconv2d(deconv2, [config.batch_size, 8, 1, config.wavenet_filters], name = "P_dec3") + conv5)

    deconv4 = tf.nn.relu(deconv2d(deconv3, [config.batch_size, 16, 1, config.wavenet_filters], name = "P_dec4") + conv1)

    deconv5 = tf.nn.relu(deconv2d(deconv4, [config.batch_size, 32, 1, config.wavenet_filters], name = "P_dec5") +  inputs)
...
def GAN_discriminator(inputs, singer_label, phones, f0_notation):
    ...
    conv1 =  tf.nn.leaky_relu(tf.layers.conv2d(inputs, 64, (8,8), strides=(1,1),  padding = 'same', name = "G_1", kernel_initializer=tf.random_normal_initializer(stddev=0.02)))
...
def GAN_generator(singer_label, phones, f0_notation, rand):
    ...
    deconv1 = tf.image.resize_images(conv8, size=(2,1), method=tf.image.ResizeMethod.NEAREST_NEIGHBOR)
    ...
    deconv2 = tf.image.resize_images(deconv1, size=(4,1), method=tf.image.ResizeMethod.NEAREST_NEIGHBOR)
    ...
    deconv3 = tf.image.resize_images(deconv2, size=(8,1), method=tf.image.ResizeMethod.NEAREST_NEIGHBOR)
    ...
    deconv4 = tf.image.resize_images(deconv3, size=(16,1), method=tf.image.ResizeMethod.NEAREST_NEIGHBOR)
    ...
    deconv5 = tf.image.resize_images(deconv4, size=(32,1), method=tf.image.ResizeMethod.NEAREST_NEIGHBOR)
